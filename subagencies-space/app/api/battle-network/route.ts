...
function bannedCreators(value: unknown) {
  const items = (value && typeof value === "object" ? value as Record<string, unknown> : {}).bannedCreators;
  return Array.isArray(items) ? [...new Set(items.map(creatorKey).filter(Boolean))] : [];
}
function banned(items: string[], creator: unknown) { return items.includes(creatorKey(creator)); }
type CreatorBlock = { creator: string; agencyIds: string[] };
function creatorBlocks(value: unknown): CreatorBlock[] { const items = (value && typeof value === "object" ? value as Record<string, unknown> : {}).creatorBlocks; return Array.isArray(items) ? items.flatMap((item): CreatorBlock[] => { const row = item && typeof item === "object" ? item as Record<string, unknown> : {}; const creator = creatorKey(row.creator); const agencyIds = Array.isArray(row.agencyIds) ? [...new Set(row.agencyIds.map(key).filter(Boolean))] : []; return creator && agencyIds.length ? [{ creator, agencyIds }] : []; }) : []; }
...
export async function POST(request: Request) {
  try {
    ...
    if (["claim-battle", "external-claim", "add-manual-opponent"].includes(body.action)) {
      ...
      if (agencyError) throw new Error(agencyError.message); if (!agency || !username) return NextResponse.json({ error: "COMPLETE THE OPPONENT DETAILS." }, { status: 409 }); const currentSettings = await settings(); if (banned(bannedCreators(currentSettings), source.creatorUsername) || banned(bannedCreators(currentSettings), username)) return NextResponse.json({ error: "THIS CREATOR IS BANNED FROM BATTLES." }, { status: 409 }); if (await hasDuplicateBattle({ agency_id: agencyId, week_start: source.weekStart, day: source.day, creator_username: username, requested_time: source.requestedTime })) return NextResponse.json({ error: "A BATTLE FOR THIS CREATOR AT THIS TIME ALREADY EXISTS." }, { status: 409 });
