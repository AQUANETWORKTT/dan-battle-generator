...
function createLocalTemplate(): PosterTemplateRow {
  return {
    ...
    name: "New Template",
    background_url: null,
    template_json: createBlankTemplateJson(),
  };
}
...
const BRAND = {
  name: "Paradise Poster Generator",
  manager: "PARADISE",
  posterBackground: "/posters/dan-battle/background.png",
  zipName: "Paradise-Posters.zip",
};

const AGENCY_GENERATOR_THEMES: Record<string, { name: string; color: string; background: string }> = {
  paradise: { name: "Paradise", color: "#facc15", background: "/agency-page-backgrounds/paradise.png" },
  respawn: { name: "Respawn", color: "#28d7c3", background: "" },
  horizon: { name: "Horizon", color: "#f97316", background: "/agency-page-backgrounds/horizon.png" },
  trident: { name: "Trident", color: "#2563eb", background: "/agency-page-backgrounds/trident.png" },
};
...
export default function BattleGeneratorPage() {
  const routeParams = useParams<{ agency: string }>();
  const agencyId = routeParams.agency?.toLowerCase() || "paradise";
  const agencyTheme = AGENCY_GENERATOR_THEMES[agencyId] || AGENCY_GENERATOR_THEMES.paradise;
  const agencyTemplatePrefix = `subagency:${agencyId}:`;
  const defaultTemplateStorageKey = `${DEFAULT_TEMPLATE_STORAGE_KEY}-${agencyId}`;
  const defaultTemplateSettingKey = `${DEFAULT_TEMPLATE_SETTING_KEY}-${agencyId}`;
  const agencyBrand = { ...BRAND, name: `${agencyTheme.name} Poster Generator`, manager: agencyTheme.name.toUpperCase(), zipName: `${agencyTheme.name}-Posters.zip` };
  ...
  function getBrowserDefaultTemplateId() {
    ...
    return window.localStorage.getItem(defaultTemplateStorageKey) || "";
  }

  function rememberBrowserDefaultTemplateId(templateId: string) {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(defaultTemplateStorageKey, templateId);
  }
  ...
  async function getPublicDefaultTemplateId(supabase: ReturnType<typeof getPosterSupabaseClient>) {
    ...
    const { data, error } = await supabase
      ...
      .select("template_id")
      .eq("setting_key", defaultTemplateSettingKey)
      .maybeSingle();
  ...
  async function setCurrentTemplateAsDefault() {
    ...
    const { error } = await supabase
      ...
      .upsert(
        {
          setting_key: defaultTemplateSettingKey,
  ...
  async function loadPosterTemplates() {
    ...
    const { data, error } = await supabase
      ...
      .like("name", `${agencyTemplatePrefix}%`)
    ...
    const rows = data.map((row) => ({
      ...row,
      name: String(row.name).slice(agencyTemplatePrefix.length),
    ...
    try {
      if (selectedTemplateId === "local-default" || selectedTemplateId.startsWith("local-")) {
        const { data, error } = await supabase
          .from("poster_templates")
          .insert({
            name: `${agencyTemplatePrefix}${templateName.trim()}`,
            background_url: nextJson.backgroundUrl || null,
            template_json: nextJson,
          })
          .select("id,name,background_url,template_json")
        ...
        const row = {
          ...data,
          name: String(data.name).slice(agencyTemplatePrefix.length),
  ...
  async function duplicateCurrentTemplate() {
    ...
    const { data, error } = await supabase
      .from("poster_templates")
      .insert({
        name: `${agencyTemplatePrefix}${copyName}`,
        background_url: nextJson.backgroundUrl || null,
        template_json: nextJson,
      })
      .select("id,name,background_url,template_json")
...
function renderText(
 ...
 const content = (
  ...
  return (
    <div className="agency-generator-shell min-h-screen text-white p-8" style={{ "--generator-background": agencyTheme.background ? `url(${agencyTheme.background})` : "none", "--generator-accent": agencyTheme.color } as React.CSSProperties}>
      <div className="max-w-[1700px] mx-auto space-y-6">
	<div className="flex gap-3 mb-4">
  <a
    href={`/agency/${agencyId}`}
    className="agency-generator-logo"
    aria-label={`Back to ${agencyTheme.name}`}
  >
    <img src={`/agency-logos/${agencyId}.png`} alt={`${agencyTheme.name} agency`} />
  </a>
        ...
        <div className="flex flex-col gap-4">
          <div>
            <h1 className="text-yellow-300 text-3xl font-black tracking-[0.18em] uppercase">
              {isPostersWorkspace ? "Posters" : isCrewShowdownWorkspace ? "Crew Showdown Downloads" : agencyBrand.name}
            </h1>
