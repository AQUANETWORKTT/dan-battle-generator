const COUNTRY_RANKINGS_URLS = {
  gb: "https://www.tikleap.com/country/gb",
  au: "https://www.tikleap.com/country/au",
};
...
chrome.runtime.onMessage.addListener((message, sender) => {
  ...
  if (message?.type === "pull-uk-live-leagues" && sender.tab?.id) {
    const dataSpaceTabId = sender.tab.id;
    const country = message.country === "au" ? "au" : "gb";
    const countryUrl = COUNTRY_RANKINGS_URLS[country];
    ...
    (async () => {
      ...
      try {
        ...
        for (const league of requestedLeagues) {
          const url = `${countryUrl}/league/${league.toLowerCase()}`;
        ...
        sendToDataSpace(dataSpaceTabId, { type: "league-rankings-complete", leagues: requestedLeagues, country });
      } catch (error) {
  ...
  (async () => {
    try {
      ...
      const ukUrl = COUNTRY_RANKINGS_URLS.gb;
      let tickleapTab = tabs.find((candidate) => candidate.url?.startsWith(ukUrl));
      if (!tickleapTab?.id) {
        tickleapTab = await chrome.tabs.create({ url: ukUrl, active: false });
