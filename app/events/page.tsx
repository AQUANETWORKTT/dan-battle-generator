...
type EventCard = {
  ...
  creatorHref?: string;
  tone: "pink" | "yellow" | "emerald";
  finished?: boolean;
};

const currentEvents: EventCard[] = [];

const previousEvents: EventCard[] = [
  { name: "Race to the Top", status: "Event finished", logo: "/race-to-the-top-logo-transparent.png", tone: "pink", finished: true },
...
function EventGallery({ title, description, events, copyCreatorLink, previous = false }: { title: string; description: string; events: EventCard[]; copyCreatorLink: (path: string) => void; previous?: boolean }) {
            ...
            return (
              <article key={event.name} className={`group overflow-hidden rounded-[28px] border p-5 transition ${previous ? "border-white/10 bg-white/[0.02] opacity-55 grayscale hover:opacity-80" : "border-white/10 bg-white/[0.04] hover:-translate-y-1 hover:border-yellow-300/45"}`}>
                <div className={`relative flex h-52 items-center justify-center overflow-hidden rounded-2xl border border-white/5 p-7 ${green ? "bg-emerald-900/20" : pink ? "bg-pink-500/20" : "bg-yellow-300/10"}`}>
                  ...
                  {event.logo ? <Image src={event.logo} alt={event.name} className="relative max-h-full max-w-full object-contain drop-shadow-2xl" width={480} height={320} /> : <span className="flex h-24 w-24 items-center justify-center rounded-xl border-4 border-red-300 bg-red-600 text-center text-[10px] font-black uppercase tracking-[.14em] text-red-50">Logo<br/>placeholder</span>}
                </div>
                <div className="mt-5">
                ...
                <div className="mt-6 grid gap-2">
                  {event.finished ? (
                    <span className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-center text-xs font-black uppercase tracking-wide text-white/35">This event has finished</span>
                  ) : (
                    <>
