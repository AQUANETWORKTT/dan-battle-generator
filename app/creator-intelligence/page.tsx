...
const FIRST_CLASS_MANAGER_CONFIG: Record<string, { name: string; group: string }> = {
  ...
  firstclassagencyashoutlookcom: { name: "Ash", group: "Team Dan / James" },
...
async function renderTeamHealthPosterToPngBlob(managerSummary: ManagerHealthSummary, excludedUsernames: Set<string>) {
    ...
    .map((creator, index) => {
      ...
      return `
        <div style="display:grid;grid-template-columns:46px minmax(210px,1fr) 112px 116px 116px 112px 120px 108px 156px;align-items:center;min-height:56px;border-left:2px solid ${tone.border};border-right:2px solid ${tone.border};border-bottom:1px solid ${tone.border}99;background:linear-gradient(90deg,rgba(3,3,3,.96),rgba(17,17,17,.9));box-shadow:0 0 18px ${tone.border}33 inset;">
          ...
          <div style="text-align:center;color:#fff7ed;font-weight:950;"><div style="font-size:18px;line-height:1;">${formatNumber(creator.healthWindowMatches)}/70</div><div style="margin-top:3px;font-size:10px;color:#facc15;letter-spacing:1px;">${formatNumber(creator.healthBreakdown.matches)} PTS</div></div>
          <div style="text-align:center;color:#fff7ed;font-weight:950;"><div style="font-size:18px;line-height:1;">${formatNumber(creator.dph)}</div><div style="margin-top:3px;font-size:10px;color:#facc15;letter-spacing:1px;">${formatNumber(creator.healthBreakdown.dph)} PTS</div></div>
  ...
  const healthScoreRules = [
    ...
    .join("");
  const posterHtml = `
    <div style="position:relative;width:1480px;min-height:720px;overflow:hidden;box-sizing:border-box;padding:26px 28px 34px;background:
      ...
      <div style="position:relative;z-index:1;text-align:center;">
        ...
        <div style="margin-top:7px;font-size:14px;font-weight:950;letter-spacing:3px;color:#fde68a;text-transform:uppercase;">Based on the last 7 days</div>
      </div>
...
export default function CreatorIntelligencePage() {
  ...
  const [expandedRecruitmentManager, setExpandedRecruitmentManager] = useState("");
          ...
          <div className="mt-5 overflow-x-auto">
            <div className="min-w-[720px] overflow-hidden rounded-2xl border border-slate-200">
              ...
              {visibleRecruitmentQuality.map((item, index) => {
                ...
                const expanded = expandedRecruitmentManager === item.key;
                const recruits = recruitmentCreators.filter((creator) => normalizeManagerKey(creator.managerRaw) === item.key).sort((a, b) => b.dph - a.dph || b.diamonds - a.diamonds);
                return (
                  <div key={item.key} className="border-t border-slate-100"><div className="grid grid-cols-[52px_minmax(220px,1.8fr)_1fr_1fr_1fr] items-center gap-3 px-4 py-3 text-sm"><span className="text-lg font-black text-violet-700">{index + 1}</span><div className="flex items-center gap-2"><button type="button" onClick={() => setExpandedRecruitmentManager(expanded ? "" : item.key)} disabled={!item.recruits} aria-label={`Show ${item.manager} recruits`} className="grid h-6 w-6 shrink-0 place-items-center rounded-md border border-violet-200 bg-violet-50 text-xs font-black text-violet-700 disabled:opacity-30">{expanded ? "⌄" : "›"}</button><div><p className={`font-black ${noRecruitClass}`}>{item.manager}</p><p className="mt-0.5 text-xs font-bold text-slate-400">{item.group}</p></div></div><span className={`font-black ${item.recruits === 0 ? "text-red-600" : "text-slate-700"}`}>{formatNumber(item.recruits)}</span><span className="font-black text-slate-700">{formatNumber(item.diamonds)}</span><span className={`w-fit rounded-full px-3 py-1 font-black ${dphClass}`}>{formatNumber(item.averageDph)} DPH</span></div>{expanded ? <div className="border-t border-violet-100 bg-violet-50/60 px-4 py-3"><div className="grid grid-cols-[minmax(220px,1.6fr)_1fr_1fr_1fr] gap-3 text-[10px] font-black uppercase tracking-wide text-violet-700"><span>Creator</span><span>Day Joined</span><span>Diamonds</span><span>DPH</span></div>{recruits.map((creator) => { const recruitDphClass = creator.dph >= 2500 ? "text-emerald-700" : creator.dph >= 2000 ? "text-orange-700" : creator.dph >= 1000 ? "text-yellow-700" : "text-red-700"; return <div key={creator.key} className="mt-2 grid grid-cols-[minmax(220px,1.6fr)_1fr_1fr_1fr] gap-3 border-t border-violet-100 pt-2 text-sm"><span className="font-black text-slate-900">{creator.username}</span><span className="font-black text-slate-700">DAY {creator.daysSinceJoining}</span><span className="font-black text-slate-700">{formatNumber(creator.diamonds)}</span><span className={`font-black ${recruitDphClass}`}>{formatNumber(creator.dph)} DPH</span></div>; })}</div> : null}</div>
                );
              })}
              {!visibleRecruitmentQuality.length ? <p className="p-4 text-sm text-slate-400">No manager recruitment matches the selected groups.</p> : null}{expandedRecruitmentManager ? <div className="border-t border-violet-100 bg-violet-50 px-4 py-3"><p className="text-[10px] font-black uppercase tracking-wide text-violet-700">Open TikTok Profiles</p><div className="mt-2 flex flex-wrap gap-2">{expandedRecruitmentCreators.map((creator) => <a key={creator.key} href={`https://www.tiktok.com/@${creator.username.replace(/^@/, "")}`} target="_blank" rel="noreferrer" className="rounded-lg border border-violet-200 bg-white px-3 py-2 text-[10px] font-black uppercase text-violet-700 hover:bg-violet-100">View {creator.username}</a>)}</div></div> : null}</> : <><div className="grid grid-cols-[52px_minmax(140px,1fr)_minmax(140px,1fr)_1fr_1fr_1fr_auto] gap-3 bg-slate-950 px-4 py-3 text-xs font-black uppercase tracking-wide text-white"><span>Rank</span><span>Manager</span><span>Creator</span><span>Day Joined</span><span>Diamonds</span><span>DPH</span><span>Profile</span></div>{visibleRecruitmentCreators.map((creator, index) => { const dphClass = creator.dph >= 2500 ? "bg-emerald-100 text-emerald-800" : creator.dph >= 2000 ? "bg-orange-100 text-orange-800" : creator.dph >= 1000 ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"; return <div key={creator.key} className="grid grid-cols-[52px_minmax(140px,1fr)_minmax(140px,1fr)_1fr_1fr_1fr_auto] items-center gap-3 border-t border-slate-100 px-4 py-3 text-sm"><span className="text-lg font-black text-violet-700">{index + 1}</span><div><p className="font-black text-slate-900">{creator.managerLabel}</p><p className="mt-0.5 text-xs font-bold text-slate-400">{creator.managerGroup}</p></div><span className="font-black text-slate-900">{creator.username}</span><span className="font-black text-slate-700">DAY {creator.daysSinceJoining}</span><span className="font-black text-slate-700">{formatNumber(creator.diamonds)}</span><span className={`w-fit rounded-full px-3 py-1 font-black ${dphClass}`}>{formatNumber(creator.dph)} DPH</span><a href={`https://www.tiktok.com/@${creator.username.replace(/^@/, "")}`} target="_blank" rel="noreferrer" className="rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-[10px] font-black uppercase text-violet-700 hover:bg-violet-100">View Profile</a></div>; })}{!visibleRecruitmentCreators.length ? <p className="p-4 text-sm text-slate-400">No recruited creators match the selected groups.</p> : null}</>}
            </div>
          </div>
        </section>
