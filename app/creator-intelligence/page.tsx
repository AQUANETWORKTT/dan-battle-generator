        ...
        <section className="mt-6 rounded-3xl border border-violet-100 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 overflow-x-auto">
            <div className="min-w-[720px] overflow-hidden rounded-2xl border border-slate-200">
              ...
              {!visibleRecruitmentQuality.length ? <p className="p-4 text-sm text-slate-400">No manager recruitment matches the selected groups.</p> : null}</> : <><div className="grid grid-cols-[52px_minmax(150px,1.1fr)_minmax(150px,1.1fr)_1fr_1fr_1fr] gap-3 bg-slate-950 px-4 py-3 text-xs font-black uppercase tracking-wide text-white"><span>Rank</span><span>Manager</span><span>Creator</span><span>Day Joined</span><span>Diamonds</span><span>DPH</span></div>{visibleRecruitmentCreators.map((creator, index) => { const dphClass = creator.dph >= 2500 ? "bg-emerald-100 text-emerald-800" : creator.dph >= 2000 ? "bg-orange-100 text-orange-800" : creator.dph >= 1000 ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"; return <div key={creator.key} className="grid grid-cols-[52px_minmax(150px,1.1fr)_minmax(150px,1.1fr)_1fr_1fr_1fr] items-center gap-3 border-t border-slate-100 px-4 py-3 text-sm"><span className="text-lg font-black text-violet-700">{index + 1}</span><div><p className="font-black text-slate-900">{creator.managerLabel}</p><p className="mt-0.5 text-xs font-bold text-slate-400">{creator.managerGroup}</p></div><span className="font-black text-slate-900">{creator.username}</span><span className="font-black text-slate-700">DAY {creator.daysSinceJoining}</span><span className="font-black text-slate-700">{formatNumber(creator.diamonds)}</span><span className={`w-fit rounded-full px-3 py-1 font-black ${dphClass}`}>{formatNumber(creator.dph)} DPH</span></div>; })}{!visibleRecruitmentCreators.length ? <p className="p-4 text-sm text-slate-400">No recruited creators match the selected groups.</p> : null}</>}
            </div>
          </div>
        </section>
