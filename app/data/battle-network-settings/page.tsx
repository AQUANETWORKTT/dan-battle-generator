...
export default function BattleNetworkSettingsPage() {
  ...
  function addManager(agencyId: string) { const draft = managerDrafts[agencyId]; if (!draft?.name?.trim() || !draft.initials?.trim()) return; const manager = { name: draft.name.trim(), initials: draft.initials.trim().toUpperCase().slice(0, 5) }; const index = editingManager[agencyId]; setManagerSettings((current) => ({ ...current, [agencyId]: index === undefined ? [...(current[agencyId] || []), manager] : (current[agencyId] || []).map((item, itemIndex) => itemIndex === index ? manager : item) })); setManagerDrafts((current) => ({ ...current, [agencyId]: { name: "", initials: "" } })); setEditingManager((current) => ({ ...current, [agencyId]: undefined })); }
