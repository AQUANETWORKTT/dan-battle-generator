...
type LookupResult = { creatorId: string; username: string; manager: string; joinedDate: string; count: number; rows: StatRow[] };
...
export default function CreatorIdLookupPage() {
  ...
  return <DataAccessGuard><main className="min-h-screen bg-[#080806] px-5 py-6 text-white sm:px-8 sm:py-8"><div className="mx-auto max-w-7xl">
    ...
    {result ? <section className="mt-7"><div className="flex flex-wrap items-end justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-5"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/45">Matching account</p><h2 className="mt-2 font-[family-name:var(--font-norwester)] text-3xl uppercase text-sky-200">{result.username || "Username unavailable"}</h2><p className="mt-2 font-mono text-sm text-white/60">Creator ID: {result.creatorId}</p><div className="mt-4 flex flex-wrap gap-3 text-sm"><p className="rounded-lg border border-white/10 bg-black/25 px-3 py-2"><span className="text-white/45">Manager: </span><strong>{result.manager || "Not recorded"}</strong></p><p className="rounded-lg border border-white/10 bg-black/25 px-3 py-2"><span className="text-white/45">Joined: </span><strong>{result.joinedDate || "Not available"}</strong></p></div></div><p className="rounded-full border border-sky-300/25 bg-sky-300/10 px-4 py-2 text-xs font-black uppercase tracking-wider text-sky-100">{result.count} most recent record{result.count === 1 ? "" : "s"}</p></div>
