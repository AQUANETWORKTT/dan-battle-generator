...
function DiamondBar({ value, target, maintenance, rankUp, pace }: { value: number; target: number; maintenance: number | null; rankUp: number | null; pace: number }) {
  ...
  return <div><div className="mb-1 text-[10px] font-black uppercase text-white/70">Diamonds: {value.toLocaleString("en-GB")} / {target.toLocaleString("en-GB")}</div><div className="relative pt-5"><div className="relative h-3 overflow-hidden rounded-sm bg-black/35"><div className={`h-full ${paceColour(pace)}`} style={{ width: `${Math.round(completed * 100)}%` }} />{[...grouped.entries()].map(([markerValue, labels]) => { const position = Math.min(100, Math.max(0, markerValue / maximum * 100)); const achieved = value >= markerValue; return <div key={markerValue} title={`${labels.join("/")} · ${markerValue.toLocaleString("en-GB")}`} className={`absolute inset-y-0 z-10 w-px ${achieved ? "bg-emerald-300" : "bg-white/90"}`} style={{ left: `${position}%` }} />; })}</div>{[...grouped.entries()].map(([markerValue, labels]) => { const position = Math.min(100, Math.max(0, markerValue / maximum * 100)); const achieved = value >= markerValue; return <div key={`label-${markerValue}`} title={`${labels.join("/")} · ${markerValue.toLocaleString("en-GB")}`} className={`absolute top-0 z-20 -translate-x-1/2 text-[9px] font-black tracking-wide ${achieved ? "text-emerald-300" : "text-white"}`} style={{ left: `${position}%` }}><span className="rounded bg-black/70 px-1">{labels.join("/")}</span></div>; })}</div></div>;
}
...
export default function Page() {
  ...
  useEffect(() => { let active = true; fetch(`/api/data-analysis/daily-stats?month=${previousMonth}&t=${Date.now()}`, { cache: "no-store" }).then(async response => { const body = await response.json(); if (!response.ok) throw Error(body.error || "Could not load prior-period targets."); if (active) setPreviousRows(body.rows || []); }).catch(e => active && setError(e.message)); return () => { active = false; }; }, [previousMonth]);
  ...
  const creators = useMemo(() => { const collect = (source: Row[], zeroProgress = false) => { const latestDate = source.reduce((latest, row) => String(row.stat_date || "") > latest ? String(row.stat_date) : latest, ""); const grouped = new Map<string, Row[]>(); for (const row of source) { const id = creatorId(row); if (id) grouped.set(id, [...(grouped.get(id) || []), row]); } return [...grouped.values()].flatMap((group): Creator[] => { const latest = group.find(row => row.stat_date === latestDate); if (!latest || !isDan(latest) || !username(latest)) return []; const monthTotal = [...group].sort((a, b) => String(b.stat_date).localeCompare(String(a.stat_date))).find(isMonthTotal); const daily = group.filter(row => !isMonthTotal(row)); const hours = zeroProgress ? 0 : monthTotal ? liveHours(monthTotal) : daily.reduce((sum, row) => sum + liveHours(row), 0); const diamonds = zeroProgress ? 0 : monthTotal ? number(monthTotal.diamonds) : daily.reduce((sum, row) => sum + number(row.diamonds), 0); const declaredDays = monthTotal ? number(monthTotal.valid_days ?? monthTotal.valid_live_days) : 0; const days = zeroProgress ? 0 : declaredDays || daily.reduce((sum, row) => sum + (number(row.valid_days ?? row.valid_live_days) || (liveHours(row) >= 1 ? 1 : 0)), 0); return [{ id: creatorId(latest), username: username(latest), days, hours, diamonds }]; }); }; const previous = collect(previousRows, true); const current = collect(rows); return [...new Map([...previous, ...current].map(creator => [creator.id, creator])).values()]; }, [rows, previousRows]);
  const previousDiamonds = useMemo(() => {
    const grouped = new Map<string, Row[]>();
    for (const row of previousRows) { const id = creatorId(row); if (id) grouped.set(id, [...(grouped.get(id) || []), row]); }
    return new Map([...grouped.values()].flatMap((group): [string, number][] => {
      const ordered = [...group].sort((a, b) => String(b.stat_date).localeCompare(String(a.stat_date)));
      const latest = ordered[0];
      const id = latest ? creatorId(latest) : "";
      if (!latest || !isDan(latest) || !id) return [];
      const total = ordered.find(isMonthTotal);
      const diamonds = total ? number(total.diamonds) : group.filter(row => !isMonthTotal(row)).reduce((sum, row) => sum + number(row.diamonds), 0);
      return [[id, diamonds]];
    }));
  }, [previousRows]);
  ...
  const setDiamonds = (name: string, diamonds: string) => {
    ...
    diamondTimers.current[key] = window.setTimeout(() => {
      ...
      const saved = current.targets[key] || {};
      const level = LEVELS.find(item => item.level === Number(saved.level)) || LEVELS[2];
      const next = { ...current, targets: { ...current.targets, [key]: { level: level.level, days: level.days, hours: level.hours, diamonds: Math.max(0, number(diamonds)) } } };
  ...
  const totals = active.reduce((summary, creator) => ({ diamonds: summary.diamonds + creator.diamonds, target: summary.target + creator.t.diamonds, maintenance: summary.maintenance + (creator.rankUp && creator.diamonds >= creator.rankUp ? 0 : creator.maintenance && creator.diamonds >= creator.maintenance ? creator.diamonds : 0), rankUp: summary.rankUp + (creator.rankUp && creator.diamonds >= creator.rankUp ? creator.diamonds : 0) }), { diamonds: 0, target: 0, maintenance: 0, rankUp: 0 });
  const projectionDaysInMonth = new Date(Number(month.slice(0, 4)), Number(month.slice(5, 7)), 0).getDate();
  const projectionTrackedDays = Math.min(projectionDaysInMonth, Math.max(1, Number(asOf.slice(-2)) || 1));
  const projectedMonthEndDiamonds = Math.round((totals.diamonds / projectionTrackedDays) * projectionDaysInMonth);

  return <DataAccessGuard><main className="min-h-screen bg-[#080806] px-5 py-8 text-white sm:px-8"><div className="mx-auto max-w-7xl"><header className="flex flex-wrap items-end justify-between gap-5"><div><Link href="/data/menu" className="text-xs font-black uppercase tracking-widest text-yellow-200">&larr; Data Space</Link><p className="mt-7 text-xs font-black uppercase tracking-[.25em] text-sky-200">First Class Agency_Dan</p><h1 className="mt-3 font-[family-name:var(--font-norwester)] text-5xl uppercase">Target <span className="text-yellow-300">Tracker</span></h1><p className="mt-3 text-sm text-white/55">Monthly targets, ordered by current diamonds.</p></div><label className="text-xs font-black uppercase text-white/55">Calendar month<input type="month" value={month} onChange={e => setMonth(e.target.value)} className="mt-2 block rounded-xl border border-white/20 bg-black px-4 py-3 text-white" /></label></header><section className="mt-6 grid gap-3 rounded-2xl border border-yellow-300/35 bg-yellow-300/[.08] px-5 py-4 sm:grid-cols-4"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-yellow-100">Total target</p><p className="mt-1 text-xl font-black text-white">{totals.diamonds.toLocaleString("en-GB")} / {totals.target.toLocaleString("en-GB")}</p><p className="mt-1 text-xs font-black text-yellow-200">{totals.target ? Math.round(Math.min(1, totals.diamonds / totals.target) * 100) : 0}% achieved</p></div><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-yellow-100">Maintained diamonds (MTD)</p><p className="mt-1 text-xl font-black text-white">{totals.maintenance.toLocaleString("en-GB")}</p></div><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-yellow-100">Ranked-up diamonds (MTD)</p><p className="mt-1 text-xl font-black text-white">{totals.rankUp.toLocaleString("en-GB")}</p></div><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-yellow-100">Projected month-end diamonds</p><p className="mt-1 text-xl font-black text-white">{projectedMonthEndDiamonds.toLocaleString("en-GB")}</p><p className="mt-1 text-xs font-black text-yellow-200">{projectionTrackedDays} of {projectionDaysInMonth} days tracked</p></div></section>{error && <p className="mt-6 rounded-xl bg-red-500/15 p-4 text-red-100">{error}</p>}<p className="mt-8 text-xs font-black uppercase tracking-widest text-yellow-100/70">{loading ? "Loading creators..." : `${active.length} active creators - ${asOf || "latest upload"}`}</p><section className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{active.map(creator => <article key={creator.username} className={`rounded-3xl border p-5 ${paceClass(creator.pace)}`}><div className="flex items-center gap-3"><h2 className="font-black">@{creator.username}</h2><span className="text-xs font-black text-white/75">{Math.round(Math.min(1, creator.diamonds / creator.t.diamonds) * 100)}% target</span><button onClick={() => saveShared({ ...stored, deleted: [...new Set([...stored.deleted, creator.username])] })} className="ml-auto text-xs font-black uppercase text-red-200">Remove</button></div><div className="mt-5 space-y-4"><Bar label="Days" value={creator.days} target={creator.t.days} pace={creator.dayPace} /><Bar label="Hours" value={creator.hours} target={creator.t.hours} pace={creator.hourPace} /><DiamondBar value={creator.diamonds} target={creator.t.diamonds} maintenance={creator.maintenance} rankUp={creator.rankUp} pace={creator.diamondPace} /></div><div className="mt-5 grid grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)] gap-3 border-t border-white/10 pt-4"><label className="text-[10px] font-black uppercase text-white/55">Level<select value={creator.t.level} onChange={e => setLevel(creator.username, number(e.target.value))} className="mt-1 w-full rounded-lg border border-white/20 bg-black/40 px-2 py-2 text-sm text-white">{LEVELS.map(level => <option key={level.level} value={level.level}>Level {level.level}: {level.days} days / {level.hours} hours</option>)}</select></label><label className="text-[10px] font-black uppercase text-white/55">Diamonds<input key={`${month}:${creator.username}:${creator.t.diamonds}`} type="number" min="0" defaultValue={creator.t.diamonds} onChange={e => setDiamonds(creator.username, e.target.value)} className="mt-1 w-full rounded-lg border border-white/20 bg-black/40 px-2 py-2 text-sm text-white" /></label></div></article>)}</section><section className="mt-8 rounded-3xl border border-white/15 bg-white/[.035] p-5"><h2 className="font-[family-name:var(--font-norwester)] text-2xl uppercase text-yellow-200">Removed creators</h2><div className="mt-4 flex flex-wrap gap-3">{removed.length ? removed.map(creator => <div key={creator.username} className="flex items-center gap-3 rounded-xl border border-white/15 px-3 py-2"><span>@{creator.username}</span><button onClick={() => saveShared({ ...stored, deleted: stored.deleted.filter(name => name !== creator.username) })} className="rounded-lg bg-yellow-300 px-2 py-1 text-xs font-black text-black">Recover</button></div>) : <p className="text-sm text-white/45">No creators have been removed.</p>}</div></section></div></main></DataAccessGuard>;
}
