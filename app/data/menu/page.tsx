...
type Workspace = { href: string; number: string; label: string; title: string; description: string; accent: keyof typeof accentStyles; maintenance?: boolean };
...
const workspaceGroups: { title: string; description: string; workspaces: Workspace[] }[] = [
  { title: "Data Settings", description: "Controls, creator visibility and reusable daily-poster assets.", workspaces: [
  ...
  { title: "Creator Targets", description: "Track creator progress, graduation and mature-creator retention.", workspaces: [
  ...
  { title: "Creator Analysis", description: "Performance analysis, creator health and agency intelligence.", workspaces: [
...
const workspaceGroupOrder = ["Diamond Information", "Creator Targets", "Creator Analysis", "Data Settings"];
...
export default function DataMenuPage() {
  return <DataAccessGuard><main className="min-h-screen overflow-hidden bg-[#080806] px-5 py-6 text-white sm:px-8 sm:py-8"><div className="pointer-events-none fixed inset-0 opacity-50 [background:radial-gradient(circle_at_100%_0%,rgba(56,189,248,0.16),transparent_27%),radial-gradient(circle_at_0%_100%,rgba(250,204,21,0.12),transparent_30%)]" /><div className="relative mx-auto max-w-7xl">
    ...
    <section className="mt-12 space-y-12">{[...workspaceGroups].sort((a, b) => workspaceGroupOrder.indexOf(a.title) - workspaceGroupOrder.indexOf(b.title)).map((group) => <div key={group.title}><div className="mb-5"><h2 className="font-[family-name:var(--font-norwester)] text-3xl uppercase text-yellow-200">{group.title}</h2><p className="mt-2 text-sm text-white/50">{group.description}</p></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{group.workspaces.map((item) => { const style = accentStyles[item.accent]; return <Link key={item.href} href={item.href} className={`group relative min-h-72 overflow-hidden rounded-[28px] border bg-white/[0.035] p-6 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.06] ${style}`}><div className={`absolute inset-0 bg-gradient-to-br ${style.split(" ")[0]} ${style.split(" ")[1]} ${style.split(" ")[2]}`} /><div className="relative flex h-full flex-col justify-between"><div className="flex items-start justify-between gap-4"><p className={`text-[10px] font-black uppercase tracking-[0.22em] ${style.split(" ").at(-1)}`}>{item.label}</p><span className="font-[family-name:var(--font-norwester)] text-3xl text-white/15">{item.number}</span></div><div>{item.maintenance && <span className="mb-3 inline-flex rounded-full border border-yellow-300/30 bg-yellow-300/10 px-3 py-1 text-[9px] font-black uppercase tracking-[0.16em] text-yellow-100">Awaiting API</span>}<h3 className="font-[family-name:var(--font-norwester)] text-3xl uppercase leading-[0.95] tracking-tight">{item.title}</h3><p className="mt-4 max-w-sm text-sm leading-relaxed text-white/60">{item.description}</p><span className="mt-6 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-white/75">Open workspace <span className="text-lg text-yellow-200 transition-transform group-hover:translate-x-1">→</span></span></div></div></Link>; })}</div></div>)}</section>
