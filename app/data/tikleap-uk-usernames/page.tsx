...
export default function TikleapUkUsernamesPage() {
  ...
  const topFiveAvailableByLeague = useMemo(() => {
    const rowsByLeague = new Map<string, AvailabilityResult[]>();
    for (const row of sortedAvailabilityResults) {
      if (!row.available) continue;
      const rows = rowsByLeague.get(row.league) || [];
      rows.push(row);
      rowsByLeague.set(row.league, rows);
    }
    return [...rowsByLeague.entries()]
      .sort(([left], [right]) => LIVE_LEAGUES.indexOf(left) - LIVE_LEAGUES.indexOf(right))
      .map(([league, rows]) => ({ league, rows: rows.slice(0, 5) }));
  }, [sortedAvailabilityResults]);
  const topFiveAvailabilityCopyText = useMemo(() => topFiveAvailableByLeague
    .map(({ league, rows }) => `${league}\n${rows.map((row) => `${row.username} | ${row.diamondText} diamonds | Available${row.invitationType ? ` — ${row.invitationType}` : ""}`).join("\n")}`)
    .join("\n\n"), [topFiveAvailableByLeague]);
  ...
  async function copyTopFiveAvailabilityMessage() {
    if (!topFiveAvailabilityCopyText) return;
    await navigator.clipboard.writeText(topFiveAvailabilityCopyText);
    const count = topFiveAvailableByLeague.reduce((total, { rows }) => total + rows.length, 0);
    setAvailabilityMessage(`Copied the top ${count} available creators across ${topFiveAvailableByLeague.length} leagues.`);
  }
