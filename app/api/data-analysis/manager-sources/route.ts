...
function getCreatorKey(row: CreatorStat) { return getText(row, ["creator_username", "Creator's username", "creator_id", "Creator ID"]).toLowerCase(); }
...
function getCreatorIntelligenceManagerLabel(row: CreatorStat) {
...
export async function GET() {
  try {
    ...
    const managers = Array.from(latestByCreator.values()).map((row) => ({ manager_key: getManagerRaw(row), manager_label: getCreatorIntelligenceManagerLabel(row) })).filter((manager) => manager.manager_key && manager.manager_label !== "Unassigned");
