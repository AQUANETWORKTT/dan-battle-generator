...
export async function GET(request: Request) {
  ...
  try {
    ...
    const response = NextResponse.json({ agencies: ((agencies.data || []) as unknown as Record<string, unknown>[]).map(toAgency), battles: ((battles.data || []) as unknown as Record<string, unknown>[]).map(toBattle), cardLayout: layout.cardLayout, cardTypography: layout.cardTypography });
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "external-select") return NextResponse.json({ ok: true });
    if (body.action === "login") { const { data, error } = await submissionsSupabase.from("battle_network_agencies").select("id,name,accent,logo_url,external_only,password").eq("external_only", false).eq("password", String(body.password || "").trim().toUpperCase()).maybeSingle(); if (error) throw new Error(error.message); return data ? NextResponse.json({ agency: toAgency(data) }) : NextResponse.json({ error: "ACCESS DENIED." }, { status: 401 }); }
