...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "save-battle") { const row = payload(body.battle || {}); if (!row.agency_id || !row.creator_username || !row.manager || !row.requested_time) return NextResponse.json({ error: "COMPLETE BATTLE DETAILS." }, { status: 400 }); if (await hasDuplicateBattle({ ...row, week_start: String(row.week_start || "") }, String(body.battle?.id || ""))) return NextResponse.json({ error: "A BATTLE FOR THIS CREATOR AT THIS TIME ALREADY EXISTS." }, { status: 409 }); const { data, error } = await submissionsSupabase.from("battle_network_battles").upsert(row).select(BATTLE_COLUMNS).single(); if (error) throw new Error(error.message); return NextResponse.json({ battle: toBattle(data) }); }
