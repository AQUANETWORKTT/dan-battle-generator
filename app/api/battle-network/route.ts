...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "claim-battle") { const source = battles.find((entry) => entry.id === String(body.sourceId)); const agency = agencies.find((entry) => entry.id === key(String(body.agencyId || ""))); const creatorUsername = String(body.creatorUsername || "").trim().replace(/^@/, ""); if (!source || source.opponentBattleId || !agency || !creatorUsername) return NextResponse.json({ error: "THAT BATTLE IS NO LONGER AVAILABLE." }, { status: 409 }); if (source.agencyId === agency.id) return NextResponse.json({ error: "CHOOSE A CREATOR FROM A DIFFERENT AVAILABLE BATTLE." }, { status: 409 }); const newBattle: Battle = { id: crypto.randomUUID(), agencyId: agency.id, weekStart: source.weekStart, day: source.day, creatorUsername, manager: "UNASSIGNED", size: source.size, powerUps: source.powerUps, requestedTime: source.requestedTime, actualTime: source.actualTime, opponentBattleId: source.id, createdAt: new Date().toISOString() }; const next = battles.map((entry) => entry.id === source.id ? { ...entry, opponentBattleId: newBattle.id } : entry); next.push(newBattle); await writeStore({ agencies, battles: next }); return NextResponse.json({ battles: next, battle: newBattle }); }
    if (body.action === "match") { const first = battles.find((entry) => entry.id === String(body.firstId)); const second = battles.find((entry) => entry.id === String(body.secondId)); if (!first || !second || first.id === second.id || first.opponentBattleId || second.opponentBattleId) return NextResponse.json({ error: "THAT BATTLE IS NO LONGER AVAILABLE." }, { status: 409 }); if (first.day !== second.day || first.size !== second.size || first.requestedTime !== second.requestedTime || first.powerUps !== second.powerUps) return NextResponse.json({ error: "BATTLES MUST MATCH ON DAY, TIME, SIZE AND POWER-UP RULES." }, { status: 409 }); const next = battles.map((entry) => entry.id === first.id ? { ...entry, opponentBattleId: second.id } : entry.id === second.id ? { ...entry, opponentBattleId: first.id } : entry); await writeStore({ agencies, battles: next }); return NextResponse.json({ battles: next }); }
