...
type Battle = { id: string; date: string; time: string; creator: string; manager: string; size: string; opponent: string; agency: string; notified?: number[]; reminders?: Record<string, { status: "success" | "failed"; at: string }> };
...
export async function GET(request: Request) {
  ...
  const now = londonNow(); const reminders = (settings.reminderMinutes || [60, 15]).map(Number); let sent = 0; let changed = false;
  const battles = await Promise.all((settings.battles || []).map(async (battle) => {
    ...
    for (const lead of due) {
      ...
      battle.reminders = { ...(battle.reminders || {}), [String(lead)]: { status: response.ok ? "success" : "failed", at: new Date().toISOString() } };
      changed = true;
      if (response.ok) { battle.notified = [...new Set([...(battle.notified || []), lead])]; sent += 1; }
    }
    return battle;
  }));
  if (changed) await submissionsSupabase.from("poster_templates").upsert({ name: SETTINGS_NAME, template_json: { ...settings, battles }, background_url: null, updated_at: new Date().toISOString() }, { onConflict: "name" });
