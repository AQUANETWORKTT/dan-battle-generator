...
export default function DataAnalysisPage() {
  ...
  const creatorTotals = useMemo(() => {
    ...
    for (const row of filteredRows) {
      ...
      if (!existing) {
        ...
        existing.diamonds =
          safeNumber(existing.diamonds) + safeNumber(row.diamonds);
        existing.live_hours =
          safeNumber(existing.live_hours) + safeNumber(row.live_hours);
        existing.matches =
          safeNumber(existing.matches) + safeNumber(row.matches);
        existing.live_streams =
          safeNumber(existing.live_streams) + safeNumber(row.live_streams);
  ...
  const totals = useMemo(() => {
    ...
    const activeCreators = new Set(
      filteredRows
        .filter(
          (row) => safeNumber(row.live_hours) > 0 || safeNumber(row.diamonds) > 0
        )
    ...
    const weekendDiamonds = weekendRows.reduce(
      (sum, row) => sum + safeNumber(row.diamonds),
      0
    );
    const weekdayDiamonds = weekdayRows.reduce(
      (sum, row) => sum + safeNumber(row.diamonds),
      0
    );
    ...
    const weekendDrop =
      weekdayDiamonds > 0
        ? ((weekdayDiamonds - weekendDiamonds) / weekdayDiamonds) * 100
        : 0;
  ...
  const agencyCards = useMemo(() => {
    return ["First Class", "Aqua", "Respawn", "Paradise", "Strive"].map(
      (agencyName) => {
        const agencyRows = rows.filter((row) => row.agency === agencyName);

        const diamonds = agencyRows.reduce(
          (sum, row) => sum + safeNumber(row.diamonds),
          0
        );
        const hours = agencyRows.reduce(
          (sum, row) => sum + safeNumber(row.live_hours),
          0
        );
        const matches = agencyRows.reduce(
          (sum, row) => sum + safeNumber(row.matches),
          0
        );
        ...
        const active = new Set(
          agencyRows
            .filter(
              (row) =>
                safeNumber(row.live_hours) > 0 || safeNumber(row.diamonds) > 0
            )
            .map((row) => row.creator_username)
        ).size;
        ...
        <section className="mb-6 rounded-3xl border border-yellow-300/30 bg-yellow-300/10 p-6">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl bg-black/40 p-4">
              Weekend diamonds are currently{" "}
              <strong className="text-yellow-200">
                {totals.weekendDrop.toFixed(1)}%
              </strong>{" "}
              below weekday diamonds for the selected month/filter.
            </div>

            <div className="rounded-2xl bg-black/40 p-4">
              Average diamonds per live hour:{" "}
              <strong className="text-yellow-200">
                {formatNumber(totals.diamondsPerHour)}
              </strong>
              .
            </div>

            <div className="rounded-2xl bg-black/40 p-4">
              {weekendProblemRows.length} creators went live during weekdays but
              have no weekend live hours in this filter.
            </div>

            <div className="rounded-2xl bg-black/40 p-4">
              Uploaded rows loaded:{" "}
              <strong className="text-yellow-200">{rows.length}</strong>.
            </div>

            <div className="rounded-2xl bg-black/40 p-4">
              Filtered rows loaded:{" "}
              <strong className="text-yellow-200">{filteredRows.length}</strong>.
            </div>

            <div className="rounded-2xl bg-black/40 p-4">
              Unique creators shown:{" "}
              <strong className="text-yellow-200">{creatorTotals.length}</strong>.
            </div>
          </div>
        </section>
                ...
                {weekendProblemRows.length ? (
                  weekendProblemRows.map((creator) => (
                    <tr key={creator.username} className="border-t border-white/10">
                      ...
                      <td className="p-3">
                        {formatNumber(creator.weekdayDiamonds)}
                      </td>
                      <td className="p-3">
                        {formatNumber(creator.weekendDiamonds)}
                      </td>
                      <td className="p-3 text-red-300">No weekend streams</td>
                    </tr>
                  ))
                ) : (
                  <tr className="border-t border-white/10">
                    <td className="p-3 text-white/50" colSpan={6}>
                      No weekend problems found yet.
                    </td>
                  </tr>
                )}
              </tbody>
                  ...
                  creatorTotals.map((creator) => {
                    const dph =
                      safeNumber(creator.live_hours) > 0
                        ? safeNumber(creator.diamonds) /
                          safeNumber(creator.live_hours)
                        : 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3">
                          {formatNumber(safeNumber(creator.followers))}
                        </td>
                        <td className="p-3">
                          {formatNumber(safeNumber(creator.diamonds))}
                        </td>
                        <td className="p-3">
                          {formatHours(safeNumber(creator.live_hours))}
                        </td>
                        <td className="p-3">
                          {formatNumber(safeNumber(creator.matches))}
                        </td>
